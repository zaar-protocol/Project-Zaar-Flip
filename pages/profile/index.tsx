      ...
      <main className="flex-grow flex flex-col items-center relative    overflow-hidden">
        <div className="w-full max-w-5xl flex flex-col md:flex-row items-center md:items-start mb-12">
          <div className="flex items-center mb-6 md:mb-0 md:mr-8">
            ...
            <div>
              <h1 className="text-3xl font-bold text-light-green">{currentVanity
                            ? currentVanity
                            : addr?.slice(0, 4) + "..." + addr?.slice(-4)}</h1>
              <button className="btn btn-sm btn-outline bg-neutral-900 font-normal flex items-center mt-2">                
                <span className="text-gray-light"> {currentEmail ? currentEmail : ""}</span>
              </button>
            </div>
          </div>
            ...
            <div className="bg-dark-gray rounded-sm p-6 shadow-xl">
              <div className="grid grid-cols-4 sm:grid-cols-4 gap-6 text-center">
                <div className="border-r border-dark-gray-r pb-4 sm:pb-0 last:border-r-0">
                  ...
                  <p className="text-light-green font-bold text-lg sm:text-2xl">#{Math.floor(history?.length/10) || 1}</p>
                </div>
                <div className="border-r border-dark-gray-r pb-4 sm:pb-0 last:border-r-0">
                  <p className="text-gray text-sm mb-2">ROI</p>
                  <p className="text-light-green font-bold text-lg sm:text-2xl">{winnings!=0 && waged !=0 ? Math.floor((winnings/(waged))*100) : 0}%</p>
                </div>
                <div className="border-r border-dark-gray-r pb-4 sm:pb-0 last:border-r-0">
                  <p className="text-gray text-sm mb-2">PROFIT</p>
                  <p className="text-light-green font-bold text-lg sm:text-2xl">${winnings-waged}</p>
                </div>
                <div>
                  ...
                  <div className="flex items-center justify-center">
                    <Image
                      ...
                      className="mr-2"
                    />
                    <p className="text-light-green font-bold text-lg sm:text-2xl"> {xpcalcs != undefined ? Number(xpcalcs) : Number(0)}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[250px] overflow-y-scroll no-scrollbar">
              {history?.map((box) => (
                <EventBox
                  ...
                  date={box.createdAt}
                  profPicUrl={ currentProfileImage
                    ? currentProfileImage
                    : "/profile.jpg"}
                  wager={box.wager}
