...
export const EventBox = ({userName, date, profPicUrl, wager, coinsAmount, minHeads, outcome} : {userName:string, date:number, profPicUrl:string, wager: number, coinsAmount: number, minHeads:number, outcome: boolean }) => {
    console.log(date);
    const d = new Date(date);
    const today = new Date();
    const timeAgo = Math.floor((today.getTime() - d.getTime()) / (1000 * 60 * 60 * 24));
    return (
    <div className="border border-dark-gray-all rounded-sm p-4">
      ...
      <div className="flex justify-between mb-2 text-sm uppercase">
        <p className="text-gray">Coins amount: {coinsAmount}</p>
        <p className="text-gray">Min heads / tails: {minHeads}</p>
      </div>
